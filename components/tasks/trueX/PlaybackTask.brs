...
sub onPositionChanged(event)
    ...
    if m.top.displayMidRollAd
        m.test = true
        m.trueXAdsPlayed = false
        m.top.displayMidRollAd = false
        m.raf.setAdUrl(m.top.midrollAdUrl)
        m.currentAdPod = getPreroll()
        playContentStream()
    else
...
function handleAds(ads) as Boolean
    ...
    if ads <> invalid AND ads.ads.count() > 0
        ...
        if isTruexAd(firstAd) and not m.trueXAdsPlayed
            data = {
            ...
            ?"****** playerScreen ****** onStreamInfo() ----->  m.global.streamInfo ";  m.global.streamInfo
            ' stop
            if m.global.streamInfo = invalid
                m.top.truexParams = data.adParameters
            end if

            m.vmap = m.global.streamInfo
            test2 = [{
                breakId: "preroll"
                cardDuration: "15"
                cardduration: 15
                endoffset: 15
                placementHash: "a000160c96e3e7b8e7b120cb73a9360d0c568ca3"
                podindex: 0
                startoffset: 0
                ...
                if vmapEntry.startOffset <> invalid and vmapEntry.endOffset <> invalid then
                    if m.videoPlayer.position >= vmapEntry.startOffset and m.videoPlayer.position <= vmapEntry.endOffset then
                        if m.adRenderer = invalid
                            ?"****** videoPlayerScreen ****** onVideoPositionChange() -----> vmapEntry.breakId "; vmapEntry.breakId
                            ?"****** videoPlayerScreen ****** onVideoPositionChange() -----> vmapEntry.vastUrl "; vmapEntry.vastUrl
...
sub playTrueXAd(data)
    ? "TRUE[X] >>> PlaybackTask::playTrueXAd() - instantiating TruexAdRenderer ComponentLibrary... data "; data

    ' instantiate TruexAdRenderer and register for event updates
    adParams = data.adParameters
    '///////////////////// Test////////////////
    ' if m.test = true
    '     adParams = {
    '         placement_hash: "a000160c96e3e7b8e7b120cb73a9360d0c568ca3"
    '         user_id: "9f2bde85-1177-50e4-b85f-f35642c6cf38"
    '         vast_config_url: "get.truex.com/a000160c96e3e7b8e7b120cb73a9360d0c568ca3/vast/config?asnw=&cpx_url=&dimension_1=FreebieTV&dimension_2=1&flag=%2Bamcb%2Bemcr%2Bslcb%2Bvicb%2Baeti-exvt&fw_key_values=&metr=0&network_user_id=9f2bde85-1177-50e4-b85f-f35642c6cf38&prof=g_as3_truex&ptgt=a&pvrn=&resp=vmap1&slid=fw_truex&ssnw=&stream_id=983164556&stream_position=midroll&vdur=&vprn=&user_agent=Roku/DVP-11.5%20&env[]=layoutJSON&ip=47.154.80.32"
    '     } 
    '     m.test = false
    ' end if
    ? adParams
    ...
    ' adPayload.placement_hash = placement_hash
    ' adPayload = ParseJson(ReadAsciiFile(decodedData.vastPayload).trim())

    ' hardcoded values
    adPayload = m.vmap
    adPayload.ads.push(m.currentAdPod.ads[0])
